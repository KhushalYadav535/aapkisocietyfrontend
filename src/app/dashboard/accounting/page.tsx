...
export default function AccountingPage() {
      ...
      switch (activeTab) {
        ...
        case 'bank-reconciliation':
          if (!brAccount) {
            ...
            setBankReconciliation((await accountingAPI.getBankReconciliation({ account_id: brAccount, month: brMonth, year: brYear, bank_statement_balance: brStatementBalance })).data || null);
          }
          break;
      }
      if (selectedLedgerAccount) {
        openLedger(selectedLedgerAccount);
      }
    } catch (error: any) {
      console.error("Failed to fetch data:", error.response?.data || error.message, error.config?.url);
    } finally {
      setLoading(false);
    }
  };
  ...
  const openLedger = async (account: any) => {
    ...
    try {
      const res = await accountingAPI.getLedger(account.id, { from: fromDate, to: toDate });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-2 border-b border-gray-200 overflow-x-auto">
        {tabs.map(t => (
          <button key={t.key} onClick={() => { setActiveTab(t.key); setSelectedLedgerAccount(null); }} className={`flex-1 py-3 text-sm font-semibold border-b-2 flex items-center justify-center gap-2 transition-colors ${activeTab === t.key ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}`}>
      ...
      {loading ? (
        ...
        <>
          ...
          {/* CHART OF ACCOUNTS / LEDGER VIEW */}
          {activeTab === 'accounts' && (
            selectedLedgerAccount ? (
              <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                <div className="flex items-center gap-4 mb-6 pb-4 border-b border-gray-100">
                  <button onClick={() => setSelectedLedgerAccount(null)} className="p-2 text-gray-500 hover:text-indigo-600 bg-gray-50 rounded-xl transition-colors">
                    <X className="w-5 h-5" />
                  </button>
                  <div>
                    <h2 className="text-xl font-bold text-gray-900">{selectedLedgerAccount.name} <span className="text-sm font-normal text-gray-500 ml-2">({selectedLedgerAccount.code})</span></h2>
                    <p className="text-sm text-gray-500">Ledger Account Statement</p>
                  </div>
                </div>
                ...
                {isLedgerLoading ? (
                  ...
                  <div className="py-12 text-center text-gray-500">Failed to load ledger data</div>
                ) : (
                  <>
                    <div className="grid grid-cols-2 gap-4 mb-6">
                      <div className="p-4 bg-gray-50 rounded-xl border border-gray-100">
                        <p className="text-xs text-gray-500 mb-1">Opening Balance</p>
                        <p className={`text-lg font-bold ${ledgerData.opening_balance >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>
                          {formatCurrency(Math.abs(ledgerData.opening_balance))} {ledgerData.opening_balance >= 0 ? 'Dr' : 'Cr'}
                        </p>
                      </div>
                      <div className="p-4 bg-gray-50 rounded-xl border border-gray-100">
                        <p className="text-xs text-gray-500 mb-1">Closing Balance</p>
                        <p className={`text-lg font-bold ${ledgerData.entries.length > 0 ? (ledgerData.entries[ledgerData.entries.length - 1].running_balance >= 0 ? 'text-emerald-600' : 'text-red-600') : (ledgerData.opening_balance >= 0 ? 'text-emerald-600' : 'text-red-600')}`}>
                          {formatCurrency(Math.abs(ledgerData.entries.length > 0 ? ledgerData.entries[ledgerData.entries.length - 1].running_balance : ledgerData.opening_balance))}
                          {(ledgerData.entries.length > 0 ? ledgerData.entries[ledgerData.entries.length - 1].running_balance : ledgerData.opening_balance) >= 0 ? ' Dr' : ' Cr'}
                        </p>
                      </div>
                    </div>
                      ...
                      <table className="w-full text-sm">
                        <thead className="bg-gray-50 border-b border-gray-100">
                          <tr>
                            ...
                            <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500">Particulars (Voucher)</th>
                            <th className="text-right px-4 py-3 text-xs font-semibold text-gray-500">Debit (₹)</th>
                            <th className="text-right px-4 py-3 text-xs font-semibold text-gray-500">Credit (₹)</th>
                        ...
                        <tbody className="divide-y divide-gray-50">
                          {ledgerData.entries.length === 0 ? (
                            ...
                            ledgerData.entries.map((entry: any, i: number) => (
                              <tr key={i} className="hover:bg-gray-50/50">
                                <td className="px-4 py-3 text-gray-600 whitespace-nowrap">{new Date(entry.entry_date).toLocaleDateString()}</td>
                                <td className="px-4 py-3">
                                  <div className="font-medium text-gray-900">{entry.voucher_type} - {entry.voucher_number}</div>
                                  <div className="text-xs text-gray-500">{entry.voucher_narration}</div>
                                </td>
                                <td className="px-4 py-3 text-right font-medium text-emerald-600">{entry.entry_type === 'DEBIT' ? formatCurrency(entry.amount) : ''}</td>
                                <td className="px-4 py-3 text-right font-medium text-red-600">{entry.entry_type === 'CREDIT' ? formatCurrency(entry.amount) : ''}</td>
                                <td className="px-4 py-3 text-right font-semibold text-gray-900">
                                  {formatCurrency(Math.abs(entry.running_balance))}
                                  <span className="text-[10px] text-gray-500 ml-1">{entry.running_balance >= 0 ? 'Dr' : 'Cr'}</span>
                                </td>
                              </tr>
                            ))
                          )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {accounts.map(acc => (
                  <div key={acc.id} onClick={() => openLedger(acc)} className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-all cursor-pointer hover:border-indigo-200">
